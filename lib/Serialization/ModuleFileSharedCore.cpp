  ...
  while (!cursor.AtEndOfStream()) {
    ...
    case input_block::EXPLICIT_MODULE_MAP_ENTRY:
      if (explicitSwiftModuleMap && explicitClangModuleMap) {
        ...
        StringRef rest, moduleName, modulePath, moduleDocPath, moduleSourceInfoPath,
            moduleCacheKey, clangModuleMapPath, headerDependencyPath;
        std::tie(moduleName, rest) = blobData.split('\0');
        std::tie(modulePath, rest) = blobData.split('\0');
        ...
        if (clangModuleMapPath.empty()) {
          ...
          ExplicitSwiftModuleInputInfo entry(
              modulePath.str(), optStr(moduleDocPath),
          ...
          ExplicitClangModuleInputInfo entry(
              clangModuleMapPath.str(), modulePath.str(), isFramework, isSystem,
              isBridgedHeaderDependency, optStr(moduleCacheKey));
